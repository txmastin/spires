...
#define WASHOUT_STEPS      1000
#define TEST_WASHOUT_STEPS 1000

// Reservoir hyperparams (tune as you like)
static const size_t NUM_NEURONS      = 200;
...
static const double CONNECTIVITY     = (double)AVG_DEGREE / (double)(NUM_NEURONS-1);
...
int main(void){
    ...
    // --- Alpha sweep: create -> init -> train -> evaluate for each alpha ---
    FILE *fp = fopen("data/alpha_mc.dat", "w");
    if (!fp) { fprintf(stderr, "Failed to open alpha_mc.dat for writing\n"); return 1; }
    fprintf(fp, "# alpha\tMC\n");

    for (double alpha = ALPHA_START; alpha <= ALPHA_END + 1e-12; alpha += ALPHA_STEP) {
        // neuron params per alpha
        double fractional_neuron_params[] = {
            ...
            /*[5] dt     */  DT,
            /*[6] Tmem   */  TMEM,
            /*[7] bias   */  0.0 // temp until calculated
        };
        
        fractional_neuron_params[7] = bias_from_alpha(alpha);
        ...
        for (size_t t=0; t<T_test_total; t++){
            step_reservoir(res, &test_x[t*NUM_FEATURES + 0]);
        ...
        for (size_t k=0; k<K_MAX; k++){
            ...
            if(!y||!yh){ fprintf(stderr,"OOM temp\n"); free(y); free(yh); free(yhat); free_reservoir(res); fclose(fp); return 1; }
    ...
    fclose(fp);
    printf("Wrote alpha vs MC to alpha_mc.dat\n");
